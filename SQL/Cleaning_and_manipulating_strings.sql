Cleaning and manipulating strings with SQL
...
SELECT customer_id, LEFT(customer_id, LENGTH(customer_id)-6) AS cust_group
FROM customers;
...
SELECT first_name,last_name, gender, REPLACE (gender, 'M', 'Male') AS Emp_Sex
FROM employees
-- 4.2: Change F to Female in the gender column of the employees table
SELECT first_name,last_name, gender, REPLACE (gender, 'F', 'Female') AS Emp_Sex
FROM employees
...
SELECT customer_id, region, country, REPLACE(LOWER(country), 'united states', 'US') AS new_country
FROM customers
...
SELECT TRIM (trailing ' ' FROM ' Coursera-Guided-Prject ')


-- 5.3: Trim the word Coursera-Guided-Projects
SELECT TRIM (both ' ' FROM ' Coursera-Guided-Prject ')

-- 5.4: Trim the word Coursera-Guided-Projects
SELECT TRIM (' Coursera-Guided-Project ')

-- 5.5: Right trim the word Coursera-Guided-Projects
SELECT RTRIM( ' Coursera-Guided-Project ')

-- 5.6: Left trim the word Coursera-Guided-Projects
SELECT LTRIM( 'Coursera-Guided-Project ')
...
SUBSTRING (CAST(birth_date AS CHAR(10)) FOR 4) AS year_only
FROM employees;
...
SELECT customer_id, STRING_AGG(product_id, ', ') AS All_Products
FROM sales
